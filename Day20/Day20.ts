...
const part1 = (input: string) => {
    ...
    for (let i = 0; i < inpArr.length; i++) {
        const p = _.map(inpArr[i].match(p_rgx), v => +v).slice(1);
        const v = _.map(inpArr[i].match(v_rgx), v => +v).slice(1);
        const a = _.map(inpArr[i].match(a_rgx), v => +v).slice(1);
        const t = 100000000000;
        curDist = _.sum(
            _.map([
                p[0] + v[0]*t + 0.5*a[0]*Math.pow(t, 2),
                p[1] + v[1]*t + 0.5*a[1]*Math.pow(t, 2),
                p[2] + v[2]*t + 0.5*a[2]*Math.pow(t, 2)
        ], v => Math.abs(v)));
...
const part2 = (input: string) => {
    ...
    for (let t = 0; t < 100000; t++) {
        // Figure out how to make this correctly find distance given t time
        // With sample input
        const positions = _.map(inpArr, particle => {
            return _.zipWith(
                _.map(
                    particle.match(p_rgx),
                    p => +p
                ).slice(1),
                    _.zipWith(
                        _.map(
                            particle.match(v_rgx),
                            v => +v * t
                        ).slice(1),
                        _.map(particle.match(a_rgx),
                            a => +a * t
                        ).slice(1),
        ...
        const counts = _.countBy(positions);
        console.log(counts);
    }
};


const inp = `p=<-317,1413,1507>, v=<19,-102,-108>, a=<1,-3,-3>
...
const sinp = `p=< 3,0,0>, v=< 2,0,0>, a=<-1,0,0>
...
console.log(part1(sinp));
console.log(part2(sinp));