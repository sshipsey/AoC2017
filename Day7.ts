...
const part1 = (input: string) => {
    const programs = input.replace(/,|\)|\(/g, '').split('\n').map(p => p.split(' '));
    let curProgArr: string[] = [];
    let ptr = 0;
    while (true) {
        curProgArr = programs[ptr];
        ptr = _.findIndex(programs,
            (program) =>
                program.length > 2 && _.includes(_.drop(program, 3), curProgArr[0]));
...
const part2 = (input: string) => {
    ...
    const extraWeight: Weights = {};
    programs.map(p => extraWeight[p[0]] = 0);
    for (const prog of programs) {
        weights[prog[0]] = +prog[1];
    }

    for (const prog of programs) {
        if (prog.length > 2) {
            curWeightSum = _.sum(_.drop(prog, 3).map(p => weights[p]));
            weights[prog[0]] += curWeightSum;
        }
    }
    ...
    for (const prog of programs) {
        if (prog.length > 2) {
            if (_.uniq(_.drop(prog, 3).map(p => weights[p])).length > 1) {
